package com.colombus.media.messaging.outbox.worker;
...
import com.colombus.media.messaging.kafka.publisher.DomainEventPublisher;
import com.colombus.media.messaging.outbox.props.OutboxWorkerProperties;
import com.colombus.media.messaging.outbox.repository.OutboxConsumeRepository;
...
public class OutboxWorker {
    ...
    private void routeAndPublish(OutboxEventRecord rec) throws Exception {
        ...
        if ("subject.asset-upsert.v1".equals(schema) || "SubjectAssetUpsert".equals(type)) {
            String key = Jsons.firstNonBlankText(payload, "subjectId");
            publisher.publishSubjectAssetUpsert(payload, key, props.getSendTimeout().toMillis());
            return;
        }
