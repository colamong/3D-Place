...
public class TokenBroker {
    ...
    private final WebClient authTokenClient;
    ...
    public Mono<String> getToken(String audience, long ttlSec) {
        return Mono.defer(() -> {
            Instant now = clock.instant();
            Entry e = cache.get(audience);
            ...
            return authTokenClient.post()
                ...
                .map(resp -> {
                    ...
                    Instant newExp = clock.instant().plusSeconds(Math.max(10, ttlSec - 10));
                ...
                .onErrorMap(ex -> {
                    ...
                    return new IllegalStateException("Failed to exchange token for aud=" + audience, ex);
                });
        });
    }
